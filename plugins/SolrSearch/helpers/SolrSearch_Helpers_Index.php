    ...
    foreach ($item->getAllElementTexts() as $text) {
      ...
      foreach ($item->getFiles() as $file) {
        if ($file->getExtension() == 'xml') {
          ...
          $contents = file_get_contents("http://10.214.32.18/loitsut/files/original/".metadata($file,'filename'));
          // Cut out teiHeader
          $cut1 = strpos($contents, '<div type="teksti">');
          $cut2 = strpos($contents, '<div type="tiedot">');
          $cut3 = strpos($contents, '</body>');
          $teksti = substr($contents, $cut1, $cut2);
          $kommentaarit = substr($contents, $cut2, $cut3);
        }
      }

      if ($field->label == 'Format') {
        // Replace indexed text value of TEI file url field with contents of TEI file
        $text->text = strip_tags($contents);
      }

      /*if ($field->label == 'Conforms To') {
        // Replace indexed text value of TEI file url field with contents of TEI file
        $text->text = strip_tags(substr($contents, $cut2, $cut3));
      }*/
      ...
      if ($field->is_facet) {
        if ($field->label == 'Date') {
          if (substr($text->text,0,3) === '160') {
            $text->text = str_replace($text->text, '1600 - 1609', $text->text);
          } else if (substr($text->text,0,3) === '161') {
            $text->text = str_replace($text->text, '1610 - 1619', $text->text);
          } else if (substr($text->text,0,3) === '162') {
            $text->text = str_replace($text->text, '1620 - 1629', $text->text);
          } else if (substr($text->text,0,3) === '163') {
            $text->text = str_replace($text->text, '1630 - 1639', $text->text);
          } else if (substr($text->text,0,3) === '164') {
            $text->text = str_replace($text->text, '1640 - 1649', $text->text);
          } else if (substr($text->text,0,3) === '165') {
            $text->text = str_replace($text->text, '1650 - 1659', $text->text);
          } else if (substr($text->text,0,3) === '166') {
            $text->text = str_replace($text->text, '1660 - 1669', $text->text);
          } else if (substr($text->text,0,3) === '167') {
            $text->text = str_replace($text->text, '1670 - 1679', $text->text);
          }
